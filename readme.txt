...
CE-Flipper was developed under Visual Studio .NET 2003 running on Windows 2000 and tested on devices running CE2.0 and 2.11 with the .NET Compact Framework installed.
...
* a .anim file (eg, "my-anim.anim"). This file contains no information inside (as of this writing - future versions will likely store header information in it) and is just used to identify an image set as an animation.
...
By default, animations run with about 330ms of delay between frames. This can be changed in the Options menu. You can also enable looping of the animation in Options. Timing is not precise, and on a slower device, you may want to select the 110ms or 55ms fast speed.
...
* header inside the .anim file to support multiple formats (eg, flipping through stacked images, which wouldn't require loading every frame at a time)
* frame "interlacing" options (to lower filesize and keep animation speed high)
* frame blending option in the converter so we drop fewer frames
* preloading all frames for short animations
* build script for desktop Windows/Mono, which would aid testing quite a bit
* subtitles? It would be nice to have some text be able to be displayed with each frame. 
* we wait for the frame to be loaded between timer steps for some reason (frame drops?), this should be a selectable option
...
My CE2.11 devices are quite a bit nicer (especially my Jornada 680), and I have managed to get TCPMP to play some (very) low quality .mp4 files on them. They run CE-Flipper quite well, but it's quite a bit less useful there..
...
At least 2.11 has far fewer problems running .NET CF applications overall.
